...
mod tests {
    ...
    fn allowlist_loads_and_has_entries() {
        ...
        // E19.02/E19.06/E19.10/E19.15/E19.20/E19.25–E19.36 expanded curated set.
        assert!(
            list.len() >= 20700,
            "expected expanded curated allowlist (>=20700), got {}",
            list.len()
        );
...
"#;
    ...
    fn default_run_does_not_fail_ci_without_suite() {
        // E19.25–E19.36: expanded allowlist needs a larger stack in debug.
        let handle = std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                if report.suite_present {
                    ...
                    assert!(
                        pass >= 20700,
                        "expected expanded allowlist pass count >= 20700, got {pass}"
                    );
                }
            })
