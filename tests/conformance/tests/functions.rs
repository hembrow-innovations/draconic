...
#[test]
fn params_call_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/functions/params_call"),
        "missing es/functions/params_call fixture, got {ids:?}"
    );
}
...
#[test]
fn nested_capture_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/functions/nested_capture"),
        "missing es/functions/nested_capture fixture, got {ids:?}"
    );
}
...
#[test]
fn function_expr_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/functions/function_expr"),
        "missing es/functions/function_expr fixture, got {ids:?}"
    );
}
...
#[test]
fn arrow_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/functions/arrow"),
        "missing es/functions/arrow fixture, got {ids:?}"
    );
}
...
#[test]
fn default_params_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/functions/default_params"),
        "missing es/functions/default_params fixture, got {ids:?}"
    );
}
...
#[test]
fn rest_params_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/functions/rest_params"),
        "missing es/functions/rest_params fixture, got {ids:?}"
    );
}
