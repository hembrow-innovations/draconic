//! ROADMAP L05 / L05.01 / L05.02 / L05.03: `describe` / `it` / `expect` + nested hooks.
//! L05 parent locks the combined testing library surface in one Program.
...
fn surface_fixture_present() {
    ...
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "stdlib/testing/surface"),
        "missing stdlib/testing/surface fixture, got {ids:?}"
    );
}
...
fn surface_runs_js_and_native() {
    ...
    let fixture = fixtures
        .iter()
        .find(|f| f.id == "stdlib/testing/surface")
        .expect("stdlib/testing/surface");
    assert!(
        fixture.targets.contains(&Target::Js) && fixture.targets.contains(&Target::Native),
        "stdlib/testing/surface must target js and native"
    );
    for name in [
        "describe",
        "it",
        ...
        assert!(
            ...
            "L05 surface must use {name} in one Program"
        );
    }
    assert_eq!(
        fixture.expect_native.stdout.as_deref(),
        Some("true\nBbiTjaA\ntrue\ntrue\ntrue\n"),
        "L05 surface must observe globalThis names, nested-hook order, matchers, and fail messages"
    );
    assert_eq!(
        fixture.expect_native.exit, 0,
        "L05 surface must terminate with exit 0"
    );
    for r in run_fixture(fixture) {
        assert!(
            r.ok,
