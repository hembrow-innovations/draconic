...
fn basic_yield_fixture_present() {
    ...
    assert!(
        ids.iter().any(|id| *id == "es/generators/basic_yield"),
        "missing es/generators/basic_yield fixture, got {ids:?}"
    );
}
...
#[test]
fn yield_expr_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/generators/yield_expr"),
        "missing es/generators/yield_expr fixture, got {ids:?}"
    );
}
...
#[test]
fn yield_resume_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/generators/yield_resume"),
        "missing es/generators/yield_resume fixture, got {ids:?}"
    );
}
...
#[test]
fn yield_star_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/generators/yield_star"),
        "missing es/generators/yield_star fixture, got {ids:?}"
    );
}
...
#[test]
fn generator_expr_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/generators/generator_expr"),
        "missing es/generators/generator_expr fixture, got {ids:?}"
    );
}
...
#[test]
fn generator_methods_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter()
...
#[test]
fn generator_for_of_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter().any(|id| *id == "es/generators/generator_for_of"),
        "missing es/generators/generator_for_of fixture, got {ids:?}"
    );
}
...
#[test]
fn generator_return_throw_fixture_present() {
    let fixtures = load_fixtures(&fixtures_dir()).expect("load fixtures");
    let ids: Vec<_> = fixtures.iter().map(|f| f.id.as_str()).collect();
    assert!(
        ids.iter()
            .any(|id| *id == "es/generators/generator_return_throw"),
        "missing es/generators/generator_return_throw fixture, got {ids:?}"
    );
}
