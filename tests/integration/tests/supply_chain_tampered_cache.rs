//! ROADMAP R03.01: compiler integration refuses a tampered module cache.
//!
...
//! cache contents are altered after the lock pin.
...
/// R03.01: after lock pin, altering cache contents must hard-fail compile.
/// Untampered compile is the control that the fixture is valid.
#[test]
fn compile_path_refuses_tampered_module_cache() {
    ...
    compile_path(&main).expect("untampered cache must compile");

    let checkout = cache.entry_dir(lib_path, &oid).unwrap();
    fs::write(checkout.join("index.drac"), "export let answer = 666;\n").unwrap();

    let err = compile_path(&main).expect_err("tampered cache must be refused");
