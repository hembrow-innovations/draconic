...
Reviewer miss on [[slice-255-d04-workspace-tests]]. This is a budget miss, not a new ROADMAP atom. D04 / D04.02 / D01.01 stay `done` on ROADMAP.md. Next Builder must beat the oracle budget on `cargo test --workspace`, not re-open the language row.

- **caused-by**: slice-255-d04-workspace-tests
- **failed oracle**: O1
