---
id: "task-409-c04-workspace-timeout"
...
Roadmap ID **C04** (Parallel `draconic test`: multi-fixture workers; deterministic aggregate exit). Review of [[slice-234-c04]] left O4 unmet: `cargo test --workspace` timed out at 120s while `test_cmd`, `cli_test_jobs`, and `cli_test_aggregate_order` stayed green. If the hang comes from the C04 change, fix that parallel-test worker-pool and aggregate-exit surface so those checks hold. Mark C04 `done` only when those tests are green. Not C04.01–C04.02 as separate atoms, C01–C03, C05–C06, L05, or Test262 runner parallelism. Do not re-open [[slice-234-c04]].
...
[[slice-233-c04-workspace-timeout]] [[ticket-123-c04-workspace-timeout]] [[slice-234-c04]]
