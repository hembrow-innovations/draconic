---
id: "task-532-l03-workspace-timeout"
...
Roadmap ID **L03** (`Crypto: SHA-256 digest + secure random bytes`). Review of [[slice-357-l03]] left O2 unmet: `cargo test --workspace` timed out at 120s while O1 (`stdlib_crypto`) stayed green. The stdlib location still needs the L03 Loop to leave the workspace green, not only the crypto conformance fixtures for SHA-256 digest and secure random bytes. If the hang comes from the L03 change, fix that SHA-256 digest and secure-random surface so both the workspace check and those fixtures hold. Mark L03 `done` only when those tests are green. Not L03.01 SHA-256 digest, L03.02 secure random bytes (already `done`), L01 encoding, L04 compression, or L10 HMAC/AEAD. Do not re-open [[slice-357-l03]]. Do not re-file Web Crypto or Node `crypto` as the v1 surface.
...
[[slice-356-l03-workspace-timeout]] [[ticket-181-l03-workspace-timeout]] [[slice-357-l03]]
