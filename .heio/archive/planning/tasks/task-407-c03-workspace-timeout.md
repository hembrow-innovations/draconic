---
id: "task-407-c03-workspace-timeout"
...
Roadmap ID **C03** (`once` / thread-safe init; mutex only if Runtime internals need it). Review of [[slice-232-c03]] left O3 unmet: `cargo test --workspace` timed out at 120s while `concurrency_sync` and `draconic-runtime --lib` stayed green. If the hang comes from the C03 change, fix that once / thread-safe init surface so those checks hold. Mark C03 `done` only when those tests are green. Not C03.01–C03.02 as separate atoms, C01/C02/C04–C06, a user-facing shared JS heap, or a public mutex Host API. Do not re-open [[slice-232-c03]].
...
[[slice-231-c03-workspace-timeout]] [[ticket-122-c03-workspace-timeout]] [[slice-232-c03]]
