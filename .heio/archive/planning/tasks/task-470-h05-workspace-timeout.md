---
id: "task-470-h05-workspace-timeout"
...
Roadmap ID **H05** (Time, clock, timers (job-queue integrated)). Review of [[slice-295-h05]] left O2 unmet: `cargo test --workspace` timed out at 120s while O1 (`host_time`) stayed green. If the hang comes from the H05 change, fix that time, clock, and job-queue timer surface so both the workspace check and the host time harness hold. Mark H05 `done` only when those tests are green. Not H05.01 wall clock `Date.now` / host `now_ms`, H05.02 monotonic clock for durations, H05.03 `setTimeout` / `clearTimeout` via job queue, H05.04 `setInterval` / `clearInterval`, H05.05 run loop waits for due timers, H06 TCP sockets, C05 structured cancellation / timeout helpers, or H00 host I/O surface policy. Do not re-open [[slice-295-h05]].
...
[[slice-294-h05-workspace-timeout]] [[ticket-151-h05-workspace-timeout]] [[slice-295-h05]]
