---
id: "task-460-h00-workspace-timeout"
...
Roadmap ID **H00** (Host I/O surface policy: module/global shape, error model, js hard-error vs polyfill matrix). Review of [[slice-285-h00]] left O3 unmet: `cargo test --workspace` timed out at 120s while O1 (`host_policy`) and O2 (`draconic-runtime` lib) stayed green. If the hang comes from the H00 change, fix that host I/O surface policy so both the workspace check and the host policy / Runtime ABI harnesses hold. Mark H00 `done` only when those tests are green. Not H00.01 host API registry and js unsupported hard diagnostic, H00.02 Runtime ABI scaffold, H00.03 I/O bytes boundary, H01–H16 concrete host ops, H17.04 optional JS/Node bridge, or R02 permission grant/deny. Do not re-open [[slice-285-h00]].
...
[[slice-284-h00-workspace-timeout]] [[ticket-146-h00-workspace-timeout]] [[slice-285-h00]]
