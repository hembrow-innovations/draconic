---
id: "task-526-l01-oracle-timeout"
...
Roadmap ID **L01** (Encoding: UTF-8 bytes↔string, Base64, hex). Review of [[slice-351-l01]] left O1 and O2 unmet: `cargo test -p draconic-conformance --test encoding` timed out at 120s without matching `test result: ok.`, and `cargo test --workspace` also timed out at 120s. A Program can convert UTF-8 bytes↔string, Base64, and hex through the designed encoding surface, and invalid input errors rather than silently corrupting. Tests under `tests/conformance` fixtures `stdlib/encoding` lock that combined surface. If the hang comes from the L01 encoding surface, fix that surface so both checks hold. Mark L01 `done` only when those tests are green. Not L01.01 UTF-8, L01.02 Base64, or L01.03 hex (already `done`), and not L02 collections, L03 crypto, or L08 URL/query. Do not re-open [[slice-351-l01]].
...
[[slice-350-l01-oracle-timeout]] [[ticket-179-l01-oracle-timeout]] [[slice-351-l01]]
