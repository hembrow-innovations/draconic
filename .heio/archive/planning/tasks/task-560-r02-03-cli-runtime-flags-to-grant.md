---
id: "task-560-r02-03-cli-runtime-flags-to-grant"
...
[[slice-385-r02-03]] [[ticket-108-r02-03-cli-runtime-flags-to-grant]]

## Gauntlet

- **round 1**: `cargo test -p draconic-cli --test permissions` — win. `test result: ok.` 5 passed (help lists `--allow-*`; fs subset honoured and write-without-read fails closed on js and native). `cargo test -p draconic-conformance --test permissions` — win. `test result: ok.` 30 passed. `cargo test --workspace --offline` exit 0, every crate `test result: ok.` ROADMAP R02.03 `done`.
