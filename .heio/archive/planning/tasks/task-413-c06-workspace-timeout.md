---
id: "task-413-c06-workspace-timeout"
...
Roadmap ID **C06** (Optional later: shared-memory atomics (advanced; not v1 bar)). Review of [[slice-238-c06]] left O2 unmet: `cargo test --workspace` timed out at 120s while `concurrency_atomics` stayed green. If the hang comes from the C06 change, fix that shared-memory atomics surface so both checks hold. Mark C06 `done` only when those tests are green. Not C01–C05, full ECMA-262 `SharedArrayBuffer` / `Atomics` Test262 allowlist (E19 / S02), Node `worker_threads` SharedArrayBuffer identity, or a shared JS heap. Do not re-open [[slice-238-c06]].
...
[[slice-237-c06-workspace-timeout]] [[ticket-125-c06-workspace-timeout]] [[slice-238-c06]]
