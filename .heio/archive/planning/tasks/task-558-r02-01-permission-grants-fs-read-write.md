---
id: "task-558-r02-01-permission-grants-fs-read-write"
...
[[slice-383-r02-01]] [[ticket-106-r02-01-permission-grants-fs-read-write]]

## Gauntlet

- **round 1**: `cargo test -p draconic-conformance --test permissions` — win. `test result: ok.` 12 passed (grant_fs / grant_net present, explicit grants, both targets). `cargo test --workspace --offline` exit 0, every crate `test result: ok.` ROADMAP R02.01 `done`.
