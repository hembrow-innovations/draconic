---
id: "task-472-h06-workspace-timeout"
...
Roadmap ID **H06** (TCP sockets (sockets-first)). Review of [[slice-297-h06]] left O2 unmet: `cargo test --workspace` timed out at 120s while O1 (`host_tcp`) stayed green. If the hang comes from the H06 change, fix that TCP sockets (sockets-first) surface so both the workspace check and the host TCP harness hold. Mark H06 `done` only when those tests are green. Not H06.01 TCP listen bind/backlog/close/ephemeral, H06.02 TCP accept → connection handle / peer address, H06.03 TCP connect dial host:port / refused/timeout, H06.04 TCP read/write bytes / partial read / close/shutdown, H06.05 TCP loopback e2e echo, H06.06 TCP listen/accept hard-error on js, H07 async socket I/O + job queue, H08 UDP, or H00 host I/O surface policy. Do not re-open [[slice-297-h06]].
...
[[slice-296-h06-workspace-timeout]] [[ticket-152-h06-workspace-timeout]] [[slice-297-h06]]
