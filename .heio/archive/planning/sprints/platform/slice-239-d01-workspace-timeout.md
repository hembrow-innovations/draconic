---
id: "slice-239-d01-workspace-timeout"
...
Review of [[slice-240-d01]] left ROADMAP D01 unfinished: O1–O3 (`release_binaries`, `install_script`, `install_smoke`) held, but O4 `cargo test --workspace` timed out at 120s. The distribution location still needs the D01 Loop to leave the workspace green, not only the install integration suite.
...
- **Re-opening [[slice-240-d01]]**: that slice stays sealed `failed`
...
- `[[task-415-d01-workspace-timeout]]`

## See also

ROADMAP.md D01, `tests/integration/tests/release_artifact.rs`, `tests/integration/tests/install_script.rs`, `tests/integration/tests/install_smoke.rs`, `scripts/release-artifact.sh`, `scripts/install.sh`, `website/install.md`, CONTEXT.md, [[location-224-distribution]], [[slice-240-d01]], [[ticket-126-d01-workspace-timeout]].
