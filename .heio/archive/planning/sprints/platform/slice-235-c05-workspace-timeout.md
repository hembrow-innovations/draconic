---
id: "slice-235-c05-workspace-timeout"
...
Review of [[slice-236-c05]] left ROADMAP C05 unfinished: O1 (`concurrency_cancel`) and O2 (`draconic-runtime --lib`) held, but O3 `cargo test --workspace` timed out at 120s. The concurrency location still needs the C05 Loop to leave the workspace green, not only the cancel harness and runtime lib tests.
...
- **Re-opening [[slice-236-c05]]**: that slice stays sealed `failed`
...
- `[[task-411-c05-workspace-timeout]]`

## See also

ROADMAP.md C05, `tests/conformance/tests/concurrency_cancel.rs`, `tests/conformance/fixtures/concurrency/cancel`, `crates/draconic-runtime/src/host_cancel_tests.rs`, `crates/draconic-backend-llvm/src/host_cancel.rs`, CONTEXT.md, [[location-222-concurrency]], [[slice-236-c05]], [[ticket-124-c05-workspace-timeout]].
