---
id: "slice-276-f05-workspace-timeout"
...
Review of [[slice-277-f05]] left ROADMAP F05 unfinished: O1 (`ffi_link_dynamic` conformance) and O2 (`ffi_link_dynamic` integration) held, but O3 `cargo test --workspace` timed out at 120s. The ffi location still needs the F05 Loop to leave the workspace green, not only the link-dynamic harnesses.
...
- **Re-opening [[slice-277-f05]]**: that slice stays sealed `failed`
