---
id: "slice-270-f02-workspace-timeout"
...
Review of [[slice-271-f02]] left ROADMAP F02 unfinished: O1 (`ffi_callback`) held, but O2 `cargo test --workspace` timed out at 120s. The ffi location still needs the F02 Loop to leave the workspace green, not only the callback harness.
...
- **Re-opening [[slice-271-f02]]**: that slice stays sealed `failed`
...
- `[[task-446-f02-workspace-timeout]]`

## See also

ROADMAP.md F02, `tests/conformance/tests/ffi_callback.rs`, `tests/conformance/fixtures/ffi/callback`, `crates/draconic-backend-llvm`, docs/adr/0003-gc-runtime-and-dual-worlds.md, CONTEXT.md, [[location-221-ffi]], [[slice-271-f02]], [[ticket-139-f02-workspace-timeout]].
