---
id: "slice-241-d02-workspace-timeout"
...
Review of [[slice-242-d02]] left ROADMAP D02 unfinished: O1–O2 (`toolchain_pin` on `draconic-cli` and `draconic-integration-tests`) held, but O3 `cargo test --workspace` timed out at 120s. The distribution location still needs the D02 Loop to leave the workspace green, not only the toolchain-pin suite.
...
- **Re-opening [[slice-242-d02]]**: that slice stays sealed `failed`
...
- `[[task-417-d02-workspace-timeout]]`

## See also

ROADMAP.md D02, `crates/draconic-cli/tests/toolchain_pin.rs`, `tests/integration/tests/toolchain_pin.rs`, `crates/draconic-cli/src/toolchain_pin.rs`, `crates/draconic-pkg/src/toolchain.rs`, CONTEXT.md, [[location-224-distribution]], [[slice-242-d02]], [[ticket-127-d02-workspace-timeout]].
