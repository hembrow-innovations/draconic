---
id: "slice-282-f09-workspace-timeout"
...
Review of [[slice-283-f09]] left ROADMAP F09 unfinished: O1 (`wasm32_wasi` LLVM backend) and O2 (`wasm32_wasi` integration) held, but O3 `cargo test --workspace` timed out at 120s. The ffi location still needs the F09 Loop to leave the workspace green, not only the wasm32/wasi emit+link harnesses.
...
- **Re-opening [[slice-283-f09]]**: that slice stays sealed `failed`
...
- `[[task-458-f09-workspace-timeout]]`

## See also

ROADMAP.md F09, `tests/integration`, `crates/draconic-backend-llvm`, docs/adr/0002-shared-ir-dual-backends.md, CONTEXT.md, [[location-221-ffi]], [[slice-283-f09]], [[ticket-145-f09-workspace-timeout]].
