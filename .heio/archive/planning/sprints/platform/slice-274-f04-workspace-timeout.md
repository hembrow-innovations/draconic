---
id: "slice-274-f04-workspace-timeout"
...
Review of [[slice-275-f04]] left ROADMAP F04 unfinished: O1 (`ffi_link_static` conformance) and O2 (`ffi_link_static` integration) held, but O3 `cargo test --workspace` timed out at 120s. The ffi location still needs the F04 Loop to leave the workspace green, not only the link-static harnesses.
...
- **Re-opening [[slice-275-f04]]**: that slice stays sealed `failed`
