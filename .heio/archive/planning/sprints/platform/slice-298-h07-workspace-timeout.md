---
id: "slice-298-h07-workspace-timeout"
...
Review of [[slice-299-h07]] left ROADMAP H07 unfinished: O1 (`host_tcp_async`) and O2 (`draconic-runtime` lib) held, but O3 `cargo test --workspace` timed out at 120s. The host-io location still needs the H07 Loop to leave the workspace green, not only the host async TCP harness and Runtime ABI tests.
...
- **Re-opening [[slice-299-h07]]**: that slice stays sealed `failed`
...
- `[[task-474-h07-workspace-timeout]]`
