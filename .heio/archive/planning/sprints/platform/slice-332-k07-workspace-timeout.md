---
id: "slice-332-k07-workspace-timeout"
...
Review of [[slice-333-k07]] left ROADMAP K07 unfinished: O1 (`draconic-cli` build) and O2 (`draconic-pkg` ensure) held, but O3 `cargo test --workspace` timed out at 120s. The packages location still needs the K07 Loop to leave the workspace green, not only the auto-fetch / `--offline` / lock-pin crate tests.
...
- **Re-opening [[slice-333-k07]]**: that slice stays sealed `failed`
...
- `[[task-508-k07-workspace-timeout]]`

## See also

ROADMAP.md K07, `crates/draconic-cli/tests/build.rs`, `crates/draconic-pkg/src/ensure.rs`, docs/adr/0009-go-style-git-packages.md, CONTEXT.md, `.heio/planning/locations/location-220-packages.md`, [[slice-333-k07]], [[ticket-170-k07-workspace-timeout]].
