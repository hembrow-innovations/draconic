---
id: "slice-233-c04-workspace-timeout"
...
Review of [[slice-234-c04]] left ROADMAP C04 unfinished: O1 (`test_cmd`), O2 (`cli_test_jobs`), and O3 (`cli_test_aggregate_order`) held, but O4 `cargo test --workspace` timed out at 120s. The concurrency location still needs the C04 Loop to leave the workspace green, not only the parallel `draconic test` harness.
...
- **Re-opening [[slice-234-c04]]**: that slice stays sealed `failed`
...
- `[[task-409-c04-workspace-timeout]]`

## See also

ROADMAP.md C04, `crates/draconic-cli/src/cmd_test.rs`, `crates/draconic-cli/tests/test_cmd.rs`, `tests/integration/tests/cli_test_jobs.rs`, `tests/integration/tests/cli_test_aggregate_order.rs`, CONTEXT.md, [[location-222-concurrency]], [[slice-234-c04]], [[ticket-123-c04-workspace-timeout]].
