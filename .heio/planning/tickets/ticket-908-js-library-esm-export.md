...
Unknown until triage. Belongs to this project, not an active website or chrome slice. Does not rewrite [[location-220-packages]] (git-backed `draconic.toml` consumers still compile `.drac` source) or [[location-224-distribution]] (JS artifacts already exist; this asks that a library artifact be Node-importable). Not an ESCALATE. Promote later into a slice and tasks. Not AFK from this ticket.
