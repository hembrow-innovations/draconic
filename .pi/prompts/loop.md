---
description: Run the unattended draconic pi-loop (default 100×) with stall watchdog
argument-hint: "[count] [SLEEP=n] [STALL_SEC=n]"
---
...
- `STALL_SEC=<seconds>` — kill a hung iteration after this many seconds with no stdout (script default **900**)
...
2. **Stall watch (required):** the script already kills children that produce no stdout for `STALL_SEC`. You still must:
   ...
   - If the **parent** `.loop/opencode-loop.mjs` itself stops printing for longer than `STALL_SEC` (or ~15 minutes if unset) with loops still remaining, treat that as a stall: inspect, restart the remaining count, or kill and report.
