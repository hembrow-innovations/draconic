//! ROADMAP D05.02: `draconic build --lto` is documented and invokable on native.
...
#[test]
fn build_native_lto_flag_is_invokable() {
    let dir = temp_dir();
    ...
    assert_eq!(
        code, 0,
        "--lto must be accepted on native build\nstdout={stdout}\nstderr={stderr}"
    );
    assert!(
        out.is_file(),
        "LTO native binary missing at {}",
        out.display()
    );
...
#[test]
fn build_js_lto_flag_is_native_only() {
    let dir = temp_dir();
    ...
    let combined = format!("{stdout}{stderr}");
    assert!(
        combined.contains("lto") && combined.contains("native"),
        "js --lto must say the flag is native-only:\n{combined}"
    );
    assert!(
        !combined.contains("unknown option"),
        "js --lto must be recognized, not unknown:\n{combined}"
    );
