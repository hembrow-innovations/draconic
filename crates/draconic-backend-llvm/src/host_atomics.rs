...
pub(crate) fn walk_host_atomics(module: &Module) -> Option<Result<String, Diagnostic>> {
    if !is_host_atomics_module(module) {
        return None;
    }
    Some(emit_host_atomics(module))
}
...
fn classify_stmt(stmt: &Stmt, ctx: &mut ClassifyCtx) -> Option<()> {
    match stmt {
        Stmt::Declare { local, init, .. } => {
            ...
            if ctx.print_top && matches!(ty, SlotTy::Number | SlotTy::Boolean | SlotTy::String) {
...
impl<'a> Emitter<'a> {
    ...
    fn intern_cstr(&mut self, s: &str) -> String {
        ...
        let g = format!(".ha.str.{}", self.state.str_globals.len());
