//! L09: native observations for `parseMultipart` / `serializeMultipart`.
...
use draconic_runtime::{parse_multipart, serialize_multipart, MimeErrorKind, MimePart};

pub(crate) fn is_es_mime_module(module: &Module) -> bool {
    classify(module).is_some()
}

pub(crate) fn emit_es_mime(module: &Module) -> Result<String, Diagnostic> {
    let info = classify(module).ok_or_else(|| diag("internal: not an es_mime module"))?;
    let mut em = Emitter::new();
    em.emit_module(&info)?;
    Ok(em.finish())
}

pub(crate) fn walk_es_mime(module: &Module) -> Option<Result<String, Diagnostic>> {
    if !is_es_mime_module(module) {
        return None;
    }
    Some(emit_es_mime(module))
}
...
fn classify(module: &Module) -> Option<ModuleInfo> {
    ...
    if !module_has_mime_surface(module, &by_id) {
        return None;
    }
...
fn builtin_for_name(name: &str) -> Option<BuiltinId> {
    match name {
        ...
        "parseMultipart" => Some(BuiltinId::ParseMultipart),
        "serializeMultipart" => Some(BuiltinId::SerializeMultipart),
        _ => None,
    }
}

fn module_has_mime_surface(module: &Module, by_id: &HashMap<LocalId, &Local>) -> bool {
    module.body.iter().any(|s| stmt_has_mime_surface(s, by_id))
}

fn stmt_has_mime_surface(stmt: &Stmt, by_id: &HashMap<LocalId, &Local>) -> bool {
    match stmt {
        Stmt::Declare { init: Some(e), .. } | Stmt::Expr { expr: e } | Stmt::Throw { value: e } => {
            expr_has_mime_surface(e, by_id)
        }
        Stmt::Try {
            block,
            handler,
            ...
            block.iter().any(|s| stmt_has_mime_surface(s, by_id))
                || handler
                    .as_ref()
                    .is_some_and(|h| h.iter().any(|s| stmt_has_mime_surface(s, by_id)))
                || finalizer
                    .as_ref()
                    .is_some_and(|f| f.iter().any(|s| stmt_has_mime_surface(s, by_id)))
        }
        Stmt::Block { body } => body.iter().any(|s| stmt_has_mime_surface(s, by_id)),
        _ => false,
    }
}

fn expr_has_mime_surface(expr: &Expr, by_id: &HashMap<LocalId, &Local>) -> bool {
    match expr {
        Expr::Local { id, .. } => by_id
            .get(id)
            .is_some_and(|l| matches!(l.name.as_str(), "parseMultipart" | "serializeMultipart")),
        Expr::IdentName { name, .. } => {
            matches!(name.as_str(), "parseMultipart" | "serializeMultipart")
        }
        Expr::Unary { arg, .. } => expr_has_mime_surface(arg, by_id),
        Expr::Binary { left, right, .. } => {
            expr_has_mime_surface(left, by_id) || expr_has_mime_surface(right, by_id)
        }
        Expr::Conditional {
            test,
            consequent,
            ...
            expr_has_mime_surface(test, by_id)
                || expr_has_mime_surface(consequent, by_id)
                || expr_has_mime_surface(alternate, by_id)
        }
        Expr::Member {
            object, property, ..
        } => expr_has_mime_surface(object, by_id) || expr_has_mime_surface(property, by_id),
        Expr::Call { callee, args, .. } | Expr::New { callee, args, .. } => {
            expr_has_mime_surface(callee, by_id)
                || args.iter().any(|a| match a {
                    Arg::Expr(e) => expr_has_mime_surface(e, by_id),
                    _ => false,
                })
        }
        Expr::Assign { value, .. } => expr_has_mime_surface(value, by_id),
        Expr::Array { elements, .. } => elements.iter().any(|el| match el {
            ArrayElement::Expr(e) | ArrayElement::Spread(e) => expr_has_mime_surface(e, by_id),
        ...
        Expr::Object { properties, .. } => properties.iter().any(|p| match p {
            ObjectProp::Property { value, .. } | ObjectProp::Accessor { value, .. } => {
                expr_has_mime_surface(value, by_id)
            }
            ObjectProp::Spread(e) => expr_has_mime_surface(e, by_id),
        }),
        _ => false,
    }
}
...
fn part_to_js(part: MimePart) -> JsVal {
    JsVal::Object(vec![
        (
            "headers".into(),
            JsVal::Object(
                part.headers
                    .into_iter()
                    .map(|(k, v)| (k, JsVal::Str(v)))
                    .collect(),
            ),
        ),
        ("body".into(), JsVal::Str(part.body)),
    ])
}

fn parts_from_js(v: &JsVal) -> Result<Vec<MimePart>, Option<Flow>> {
    let JsVal::Array(items) = v else {
        return Err(Some(type_error(
            "serializeMultipart expects parts array and string boundary",
        )));
    };
    let mut parts = Vec::new();
    for item in items {
        let JsVal::Object(props) = item else {
            return Err(Some(type_error("serializeMultipart expects part objects")));
        };
        ...
        for (k, val) in props {
            if k == "headers" {
                match val {
                    JsVal::Object(hs) => {
                        for (hk, hv) in hs {
                            headers.push((hk.clone(), to_string_val(hv)));
                        }
                    }
                    JsVal::Undef => {}
                    _ => {
                        return Err(Some(type_error(
                            "serializeMultipart expects part headers object",
                        )))
                    }
                }
            } else if k == "body" {
                match val {
                    JsVal::Str(s) => body = Some(s.clone()),
                    _ => {
                        return Err(Some(type_error(
                            "serializeMultipart expects string part body",
                        )))
                    }
                }
            }
...
fn eval_call_fn(callee: &JsVal, args: &[JsVal]) -> Result<JsVal, Option<Flow>> {
    match callee {
        JsVal::Builtin(BuiltinId::ParseMultipart) => {
            let input = match args.first() {
                Some(JsVal::Str(s)) => s.as_str(),
                _ => {
                    return Err(Some(type_error(
                        "parseMultipart expects string body and boundary",
                    )))
                }
            };
            let boundary = match args.get(1) {
                Some(JsVal::Str(s)) => s.as_str(),
                _ => {
                    return Err(Some(type_error(
                        "parseMultipart expects string body and boundary",
            ...
            match parse_multipart(input, boundary) {
                Ok(parts) => Ok(JsVal::Array(parts.into_iter().map(part_to_js).collect())),
                Err(e) => Err(Some(match e.kind {
                    MimeErrorKind::Type => type_error(e.message),
                    MimeErrorKind::Invalid => invalid_error(e.message),
                })),
            }
        }
        JsVal::Builtin(BuiltinId::SerializeMultipart) => {
            let parts = parts_from_js(args.first().unwrap_or(&JsVal::Undef))?;
            let boundary = match args.get(1) {
                Some(JsVal::Str(s)) => s.as_str(),
                _ => {
                    return Err(Some(type_error(
            ...
            match serialize_multipart(&parts, boundary) {
                Ok(s) => Ok(JsVal::Str(s)),
                Err(e) => Err(Some(match e.kind {
                    MimeErrorKind::Type => type_error(e.message),
                    MimeErrorKind::Invalid => invalid_error(e.message),
...
fn member_get(obj: &JsVal, key: &str) -> Result<JsVal, ()> {
    match obj {
        JsVal::Builtin(BuiltinId::GlobalThis) => match key {
            "parseMultipart" => Ok(JsVal::Builtin(BuiltinId::ParseMultipart)),
            "serializeMultipart" => Ok(JsVal::Builtin(BuiltinId::SerializeMultipart)),
        ...
        JsVal::Array(items) if key == "length" => Ok(JsVal::Num(items.len() as f64)),
        JsVal::Array(items) => {
...
fn typeof_str(v: &JsVal) -> String {
    match v {
        ...
        JsVal::Builtin(BuiltinId::ParseMultipart | BuiltinId::SerializeMultipart) => {
            "function".into()
        }
    }
}
    ...
    fn emit_module(&mut self, info: &ModuleInfo) -> Result<(), Diagnostic> {
        for id in &info.user_locals {
            let v = info
                .values
                .get(id)
                .ok_or_else(|| diag("es_mime: missing value"))?;
            match v {
                ...
                _ => return Err(diag("es_mime: non-printable value")),
            }
        }
        writeln!(
            self.out,
            "; Draconic LLVM backend (L09 parseMultipart / serializeMultipart)"
        )
        .ok();
...
mod tests {
    ...
    #[test]
    fn classifies_parse_parts() {
        let src = include_str!("../../../tests/conformance/fixtures/stdlib/mime/parse_parts.drac");
        let m = compile_src(src);
        assert!(is_es_mime_module(&m), "should classify as es_mime");
        let ir = emit_es_mime(&m).expect("emit");
        ...
        for s in [
            ...
            assert!(ir.contains(s), "missing {s:?} in {ir}");
        }
    }

    #[test]
    fn classifies_invalid() {
        let src = include_str!("../../../tests/conformance/fixtures/stdlib/mime/invalid.drac");
        let m = compile_src(src);
        assert!(is_es_mime_module(&m));
        let ir = emit_es_mime(&m).expect("emit");
        assert!(ir.contains("@main"));
    }

    #[test]
    fn classifies_roundtrip() {
        let src = include_str!("../../../tests/conformance/fixtures/stdlib/mime/roundtrip.drac");
        let m = compile_src(src);
        assert!(is_es_mime_module(&m));
        let ir = emit_es_mime(&m).expect("emit");
