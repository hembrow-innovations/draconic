...
pub(crate) fn emit_host_udp(module: &Module) -> Result<String, Diagnostic> {
    let info = classify(module).ok_or_else(|| diag("internal: not a host_udp module"))?;
    let mut em = Emitter::new(module, &info);
    em.emit_module()?;
    Ok(em.finish())
}

pub(crate) fn walk_host_udp(module: &Module) -> Option<Result<String, Diagnostic>> {
    if !is_host_udp_module(module) {
        return None;
    }
    Some(emit_host_udp(module))
}
...
fn classify_side_effect(expr: &Expr, ctx: &mut ClassifyCtx) -> Option<()> {
    match expr {
        ...
        Expr::Call { callee, args, .. }
            if args.len() == 4 && is_named_callee(callee, "udpSendTo") =>
        {
            ctx.has_udp = true;
...
impl<'a> Emitter<'a> {
    ...
    fn slot_ptr(&self, id: LocalId) -> Result<String, Diagnostic> {
        let name = self
            .local_name
            .get(&id)
            .ok_or_else(|| diag("host_udp: unknown local"))?;
    ...
    fn intern_cstr(&mut self, s: &str) -> String {
        ...
        let g = format!(".str.udp.{}", self.str_globals.len());
    ...
    fn emit_check_rc(&mut self, rc: &str) -> Result<(), Diagnostic> {
        let ok = self.fresh_label("udp_ok");
        let bad = self.fresh_label("udp_err");
        let cmp = self.fresh();
    ...
    fn emit_stmt(&mut self, stmt: &Stmt) -> Result<(), Diagnostic> {
        match stmt {
            Stmt::Declare { local, init, .. } => {
                let init = init
                    .as_ref()
                    .ok_or_else(|| diag("host_udp: declare needs init"))?;
            ...
            _ => Err(diag("host_udp: unsupported stmt")),
        }
    }
        ...
        match expr {
                ...
                if args.len() == 2 && is_named_callee(callee, "udpRecvFrom") =>
            {
                let h = self.emit_handle_i64(
                    arg_expr(&args[0]).ok_or_else(|| diag("host_udp: udpRecvFrom handle"))?,
                )?;
                let max_f = self.emit_number_expr(
                    arg_expr(&args[1]).ok_or_else(|| diag("host_udp: udpRecvFrom maxLen"))?,
                )?;
                ...
                // peer addr/port unused for H08.01 payload path (null)
                writeln!(
                    self.body,
                    "  {rc} = call i32 @{}(i64 {h}, i64 {max_i}, ptr {out_data}, ptr {out_len}, ptr null, ptr null)",
                    HOST_UDP_RECVFROM.symbol
                )
                .ok();
            ...
            _ => Err(diag("host_udp: expected udpRecvFrom for DynBytes")),
        }
    }
    ...
    fn emit_expr_stmt(&mut self, expr: &Expr) -> Result<(), Diagnostic> {
        match expr {
            ...
            {
                let h = self.emit_handle_i64(
                    arg_expr(&args[0]).ok_or_else(|| diag("host_udp: closeUdp handle"))?,
                )?;
                let rc = self.fresh();
                writeln!(
                    self.body,
            ...
            {
                let h = self.emit_handle_i64(
                    arg_expr(&args[0]).ok_or_else(|| diag("host_udp: udpSendTo handle"))?,
                )?;
                let (d, n) = self.emit_bytes_ptr_len(
                    arg_expr(&args[1]).ok_or_else(|| diag("host_udp: udpSendTo data"))?,
                )?;
                let host = self.emit_string_expr(
                    arg_expr(&args[2]).ok_or_else(|| diag("host_udp: udpSendTo host"))?,
                )?;
                let port_f = self.emit_number_expr(
                    arg_expr(&args[3]).ok_or_else(|| diag("host_udp: udpSendTo port"))?,
                )?;
                ...
                writeln!(
                    self.body,
                    "  {rc} = call i32 @{}(i64 {h}, ptr {d}, i64 {n}, ptr {host}, i32 {port_i})",
                    HOST_UDP_SENDTO.symbol
                )
                .ok();
                self.emit_check_rc(&rc)
            }
                ...
                if args.len() == 1 && is_named_callee(callee, "stdoutWrite") =>
            {
                self.emit_stdout_write(
                    arg_expr(&args[0]).ok_or_else(|| diag("host_udp: stdoutWrite arg"))?,
                )
            }
            _ => Err(diag("host_udp: unsupported expr stmt")),
        }
    }
    ...
    fn emit_bytes_ptr_len(&mut self, expr: &Expr) -> Result<(String, String), Diagnostic> {
        match expr {
            ...
            Expr::Local { id, .. } => match self.slot_of.get(id) {
                ...
                _ => Err(diag("host_udp: bytes arg unsupported")),
            },
            _ => Err(diag("host_udp: bytes arg unsupported")),
        }
    }
    ...
    fn emit_stdout_write(&mut self, arg: &Expr) -> Result<(), Diagnostic> {
        match arg {
            ...
            Expr::Local { id, .. } => match self.slot_of.get(id) {
                ...
                _ => Err(diag("host_udp: stdoutWrite unsupported arg")),
            },
            _ => Err(diag("host_udp: stdoutWrite unsupported arg")),
        }
    }
    ...
    fn emit_handle_expr(&mut self, expr: &Expr) -> Result<String, Diagnostic> {
        match expr {
            ...
            {
                let port_f = self.emit_number_expr(
                    arg_expr(&args[0]).ok_or_else(|| diag("host_udp: udpBind port"))?,
                )?;
                let port_i = self.fresh();
                writeln!(self.body, "  {port_i} = fptosi double {port_f} to i32").ok();
                ...
                writeln!(
                    self.body,
                    "  {rc} = call i32 @{}(i32 {port_i}, ptr {out_h})",
                    HOST_UDP_BIND.symbol
                )
                .ok();
                self.emit_check_rc(&rc)?;
                let iv = self.fresh();
            ...
            _ => Err(diag("host_udp: expected handle expr")),
        }
    }
    ...
    fn emit_number_expr(&mut self, expr: &Expr) -> Result<String, Diagnostic> {
        match expr {
            ...
            {
                let h = self.emit_handle_i64(
                    arg_expr(&args[0]).ok_or_else(|| diag("host_udp: udpLocalPort handle"))?,
                )?;
            ...
            Expr::Member {
                ...
                match object.as_ref() {
                    ...
                    _ => Err(diag("host_udp: unsupported member number")),
                }
            }
            Expr::Local { id, .. } => {
                let ptr = self.slot_ptr(*id)?;
            ...
            _ => Err(diag("host_udp: expected number expr")),
        }
    }
    ...
    fn emit_bool_expr(&mut self, expr: &Expr) -> Result<String, Diagnostic> {
        match expr {
            ...
            _ => Err(diag("host_udp: expected bool expr")),
        }
    }
    ...
    fn emit_string_expr(&mut self, expr: &Expr) -> Result<String, Diagnostic> {
        match expr {
            ...
            _ => Err(diag("host_udp: expected string expr")),
        }
    }
    ...
    fn emit_typeof(&mut self, arg: &Expr) -> Result<String, Diagnostic> {
        match arg {
            ...
            _ => Err(diag("host_udp: typeof unsupported arg")),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn emit_udp_bind_ephemeral() {
        let m = lower_src(
            r#"
            let s = udpBind(0);
            let p = udpLocalPort(s);
            let ok = p > 0;
            closeUdp(s);
            "#,
        );
        assert!(is_host_udp_module(&m));
        let ir = emit_host_udp(&m).expect("emit");
        assert!(ir.contains("draconic_rt_host_udp_bind"), "{ir}");
        assert!(ir.contains("draconic_rt_host_udp_local_port"), "{ir}");
        assert!(ir.contains("draconic_rt_host_handle_close"), "{ir}");
    }

    #[test]
    fn emit_udp_sendto_recvfrom() {
        let m = lower_src(
            r#"
            let a = udpBind(0);
            let p = udpLocalPort(a);
            let b = udpBind(0);
            udpSendTo(b, "hi", "127.0.0.1", p);
            let msg = udpRecvFrom(a, 64);
            stdoutWrite(msg);
            let n = msg.length;
            closeUdp(a);
            closeUdp(b);
        ...
        assert!(is_host_udp_module(&m));
        let ir = emit_host_udp(&m).expect("emit");
        assert!(ir.contains("draconic_rt_host_udp_sendto"), "{ir}");
        assert!(ir.contains("draconic_rt_host_udp_recvfrom"), "{ir}");
    }
}
