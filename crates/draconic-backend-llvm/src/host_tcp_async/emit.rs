...
impl<'a> super::Emitter<'a> {
    ...
    pub(super) fn is_named(&self, callee: &Expr, name: &str) -> bool {
    ...
    pub(super) fn store_local(&mut self, id: LocalId, value: &str) -> Result<(), Diagnostic> {
    ...
    pub(super) fn capture_slot_ssa(&mut self, pos: usize) -> Result<String, Diagnostic> {
    ...
    pub(super) fn intern_str(&mut self, s: &str) -> String {
    ...
    pub(super) fn emit_bytes(&mut self, expr: &Expr) -> Result<(String, String), Diagnostic> {
        match expr {
            Expr::String { value, .. } => {
                let s = value.to_string_lossy();
                let g = self.intern_str(&s);
    ...
    pub(super) fn emit_then(&mut self, object: &Expr, args: &[Arg]) -> Result<String, Diagnostic> {
    ...
    pub(super) fn looks_like_ptr(&self, _s: &str) -> bool {
