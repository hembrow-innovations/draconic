...
/* --- Worker isolate spawn/join/terminate (C01.01 / C01.02 / C01.03 / C01.04)
   spawnWorker: allocate an isolate slot backed by an OS thread (C01.04).
   kind 0 = fn entry (path ignored); kind 1 = module path (non-empty).
   Returns handle >= 1, or -1 on failure.
   joinWorker: wait for the worker OS thread to exit; 0 success, -1 invalid
   / already-joined handle.
   terminateWorker: force-stop isolate; 0 success, -1 invalid/already-dead.
   workerOsThread: 1 if handle is a live OS thread distinct from the caller;
   0 if same thread; -1 invalid or already joined/terminated.
