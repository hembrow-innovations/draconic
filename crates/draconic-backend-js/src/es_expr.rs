//! IR expression emission to ECMAScript source text.

use std::collections::HashMap;
use std::fmt::Write as _;

use draconic_ast::{AssignOp, BinaryOp, UnaryOp, UpdateOp};
use draconic_ir::{ArrayPatternEl, Expr, LocalId, ObjectPatternEl, Pattern};

use crate::es_stmt::emit_stmt;

pub(crate) fn emit_expr(out: &mut String, expr: &Expr, names: &HashMap<LocalId, &str>) {
    match expr {
        Expr::Local { id, .. } => {
...
fn emit_pattern_prop_key(
    out: &mut String,
    key: &draconic_ir::ObjectPropKey,
    names: &HashMap<LocalId, &str>,
) {
    match key {
        draconic_ir::ObjectPropKey::Computed(e) => {
            out.push('[');
            emit_expr(out, e, names);
            out.push(']');
        }
        draconic_ir::ObjectPropKey::Static(k) => {
...
pub(crate) fn emit_array_pattern(
    out: &mut String,
...
pub(crate) fn emit_object_pattern(
    out: &mut String,
            ...
            ObjectPatternEl::Prop {
                ...
                if *shorthand {
                    if let Pattern::Local(id) = binding {
                        ...
                        emit_pattern_prop_key(out, key, names);
                        out.push_str(": ");
                        emit_pattern(out, binding, names);
                    }
                } else {
...
pub(crate) fn emit_pattern(out: &mut String, pat: &Pattern, names: &HashMap<LocalId, &str>) {
    match pat {
...
pub(crate) fn local_name<'a>(names: &HashMap<LocalId, &'a str>, id: LocalId) -> &'a str {
    names
        .get(&id)
        .copied()
