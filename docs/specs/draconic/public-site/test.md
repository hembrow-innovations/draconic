...
- **website/src/tests/site-header-primary-nav.test.ts** — `site header primary nav`
  - **How:** Root layout is skip then sticky side nav then main; side nav source has wordmark, Learn, Reference, GitHub, search, and theme toggle; home and Learn routes do not remount that chrome.
  - **Why:** Locks `public-site.chrome:primary-nav` and `public-site.chrome:odm-shell`.
