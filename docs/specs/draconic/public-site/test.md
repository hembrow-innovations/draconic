...
Existing cases in `tests/integration/tests/website_pipeline.rs` lock `public-site.markdown:subset`, `public-site.ia:learn-walkable`, `public-site.ia:reference-walkable`, `public-site.fences:shipped-must-build`, `public-site.fences:forbid-not-yet-fences`, and `public-site.nav:learn-reference-status`. Related fence cases in the same file support those locks. `draconic-web/src/tests/learn-hub-nav.test.ts` also locks `public-site.ia:learn-walkable` for the Start Learn hub and aside sequence, `public-site.chrome:current-page` for Learn chapter current-page chrome, and `public-site.a11y:distinct-nav-names` for host I/O and packages accessible names. `draconic-web/src/tests/learn-pages.test.ts` locks `public-site.ia:learn-walkable` for one Start route per Learn chapter, teaching copy, frontmatter Badge status, and `.html` hrefs rewritten to app routes. `draconic-web/src/tests/learn-prev-next.test.ts` locks `public-site.ia:learn-walkable` for article-footer prev/next that follows `learn.md`, with both landings continuing to Dual worlds. `draconic-web/src/tests/reference-hub-pages.test.ts` locks `public-site.ia:reference-walkable` for the Start Reference hub, aside sequence, and working-page routes from `draconic-web/content/*.md`, `public-site.chrome:current-page` for Reference chapter current-page chrome, and `public-site.a11y:distinct-nav-names` for host I/O and packages accessible names. `draconic-web/src/tests/reference-prev-next.test.ts` locks `public-site.ia:reference-walkable` for article-footer prev/next that follows CLI, types, Dual-world rules, host I/O, and packages, and `public-site.chrome:docs-article-order` for the related-link footer on Reference working pages. `draconic-web/src/tests/mobile-a11y.test.ts` locks `public-site.a11y:keyboard-small` for stacked wrap, visible focus, and skip-link order. `draconic-web/src/tests/docs-shell.test.ts` locks `public-site.chrome:docs-sidebar` for handbook aside, article, and status Badge chrome that is not the home hero, `public-site.chrome:docs-article-order` for kicker then heading then badge then on-page outline then related-link footer filled by LearnPage and ReferencePage, and `public-site.chrome:on-page-toc` for in-article heading links. `draconic-web/src/tests/search.test.ts` locks `public-site.search:titles-headings` for a title and heading index whose Dual worlds query reaches the Learn chapter, `public-site.search:keyboard-live` for Arrow Down, Arrow Up, Enter, and live announcement of hits or No matching pages, and `public-site.search:session` for query and result reset on navigation and Escape so leftover hits cannot keep aria-current. `draconic-web/src/tests/site-header-primary-nav.test.ts` locks `public-site.chrome:primary-nav` and `public-site.chrome:odm-shell` for sticky side-nav wordmark, Learn, Reference, and GitHub, `public-site.chrome:current-page` for hub current-page chrome, and `public-site.chrome:favicon` for a served `/favicon.ico` plus a document icon link. `draconic-web/src/tests/site-footer-and-skip-link.test.ts` locks `public-site.chrome:odm-shell` for skip-to-content, sticky side nav, and main column. Asserted with no test yet: `public-site.home:landing`, `public-site.forbid-vault-as-site`, `public-site.forbid-playground`.

`draconic-web/src/tests/document-title.test.ts` locks `public-site.chrome:document-title` for per-page document titles. Home may remain Draconic. `draconic-web/src/tests/meta-description.test.ts` locks `public-site.chrome:meta-description` for page-specific description, canonical, and Open Graph tags from existing page copy. `draconic-web/src/tests/not-found.test.ts` locks `public-site.chrome:not-found` for unknown-URL recovery. `draconic-web/src/tests/code-fence-copy.test.ts` locks `public-site.fences:copy` for copy-the-text and `public-site.fences:copy-announce` for distinct names, live confirmation, and clearing Copied. `draconic-web/src/tests/typography-and-badge.test.ts` locks `public-site.nav:learn-reference-status` for shipped-chip 4.5:1 contrast at 14px so the 4.37 light pair cannot pass.
...
- **draconic-web/src/tests/learn-pages.test.ts** — `learn pages`
...
- **draconic-web/src/tests/learn-prev-next.test.ts** — `learn prev next`
  - **How:** Sequence helper neighbors match `draconic-web/content/learn.md`; both landings next to Dual worlds; LearnPage article footer renders LearnPager; no extra stops.
...
- **draconic-web/src/tests/reference-prev-next.test.ts** — `reference prev next`
...
- **draconic-web/src/tests/typography-and-badge.test.ts** — `typography and badge`
...
- **draconic-web/src/tests/mobile-a11y.test.ts** — `mobile a11y`
...
- **draconic-web/src/tests/site-footer-and-skip-link.test.ts** — `site footer and skip link`
...
- **draconic-web/src/tests/home-hero-and-cta.test.ts** — `home hero and cta`
...
- **draconic-web/src/tests/home-sample.test.ts** — `home sample`
  ...
  - **Why:** Locks `public-site.home:landing`.
- **draconic-web/src/tests/home-features.test.ts** — `home features`
  - **How:** `/` feature grid names JavaScript, LLVM, and Dual worlds. No extra product claims.
  - **Why:** Locks `public-site.home:landing`.
- **draconic-web/src/tests/document-title.test.ts** — `document title`
...
- **draconic-web/src/tests/meta-description.test.ts** — `meta description`
  - **How:** Home, Learn, and an article route each expose a description meta tag, a canonical link, and Open Graph title, description, and url tags; copy comes from the existing homepage pitch or first markdown paragraph; canonical URLs differ per path; route files do not rewrite page h1 copy and do not add Twitter cards, og:image, or theme-color.
  - **Why:** Locks `public-site.chrome:meta-description`.
- **draconic-web/src/tests/not-found.test.ts** — `not found`
...
- **draconic-web/src/tests/code-fence-copy.test.ts** — `code fence copy`
