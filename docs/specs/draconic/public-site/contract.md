...
- `public-site.markdown:subset`: Public site pages render the markdown subset of headings, paragraphs, lists, code fences, and links from `draconic-web/content/` sources.
...
- `public-site.markdown:heading-permalinks`: A heading below the title on a Learn or Reference page is a permalink to that heading's fragment id.
  ...
  test: draconic-web/src/tests/docs-shell.test.ts
- `public-site.ia:learn-walkable`: Learn is walkable as Install, from JavaScript, from systems, Dual worlds, modules, native types, host I/O, and packages, and the two landings join at Dual worlds.
  ...
  test: draconic-web/src/tests/learn-hub-nav.test.ts
  test: draconic-web/src/tests/learn-pages.test.ts
  test: draconic-web/src/tests/learn-prev-next.test.ts
- `public-site.ia:reference-walkable`: Reference is walkable as CLI, types, Dual-world rules, host I/O, and packages.
  ...
  test: draconic-web/src/tests/reference-hub-pages.test.ts
  test: draconic-web/src/tests/reference-prev-next.test.ts
...
- `public-site.fences:copy`: A visitor can copy the text of a rendered code fence on a Learn or Reference page.
  test: draconic-web/src/tests/code-fence-copy.test.ts
...
- `public-site.nav:learn-reference-status`: Learn and Reference pages include Learn and Reference navigation and a visible shipped or not-yet status. The shipped chip's text meets 4.5:1 contrast against its fill at 14px in light and dark themes.
  test: website_pipeline_learn_and_reference_nav_and_status
  test: draconic-web/src/tests/typography-and-badge.test.ts
- `public-site.home:landing`: `/` is a language homepage with pitch and Get-started CTA, not the Learn chapter dump.
  test: draconic-web/src/tests/home-hero-and-cta.test.ts
  test: draconic-web/src/tests/home-sample.test.ts
  test: draconic-web/src/tests/home-features.test.ts
- `public-site.chrome:odm-shell`: Every page, including home, uses skip-to-content plus a sticky side nav plus a main column.
  test: draconic-web/src/tests/site-footer-and-skip-link.test.ts
  test: draconic-web/src/tests/site-header-primary-nav.test.ts
- `public-site.chrome:not-found`: An unknown URL keeps skip-to-content, sticky side nav, and footer; main has a heading that names the miss and an in-site link back to a real page; the router default Not Found paragraph is not the main content; the document title names the miss; the response is 404; no side-nav item is aria-current=page for a path that is not a page.
  test: draconic-web/src/tests/not-found.test.ts
  test: not found
...
- `public-site.chrome:document-title`: Each public page's document title names the open page so tabs distinguish destinations. Home may remain Draconic. Article titles use the page's existing heading or markdown title, not a new slogan.
  test: draconic-web/src/tests/document-title.test.ts
  test: document title
- `public-site.chrome:meta-description`: Each public page exposes a page-specific meta description, a canonical URL, and Open Graph tags that summarize that page. Descriptions use existing page copy (heading, lead, or first paragraph), not a new slogan. Site name Draconic may still appear with the page name in og:title. Home may use the existing homepage pitch.
  test: draconic-web/src/tests/meta-description.test.ts
...
- `public-site.search:titles-headings`: A visitor can find a Learn or Reference page by title, heading, or teaching-page body text.
  test: draconic-web/src/tests/search.test.ts
...
- `public-site.a11y:keyboard-small`: Primary nav and article reading work with keyboard and at a small viewport.
  test: draconic-web/src/tests/mobile-a11y.test.ts
