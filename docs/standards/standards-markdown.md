...
- **Repo-root [[CONTEXT]]** is not a vault-kind note. It does not carry this frontmatter block.
- **Archived [[ROADMAP]]** at `docs/overview/ROADMAP.md` keeps its historical pipe tables. Do not convert them in passing.
- **Public site sources** in the sibling `draconic-web` repo use a smaller markdown subset and status frontmatter ([[0010-public-docs-draconic-ssg]], [[0013-public-site-tanstack-start]], [[guides-public-docs]]). They are not vault notes.
