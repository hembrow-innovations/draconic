...
- **build**: `draconic build --target js|native [--watch] [--offline] [--library] [--strip|--strip-symbols] [--lto] [--link <lib.a>] <file> [-o|--out|--output <out>]`. `--target` is required (`--target=` accepted). Default output is `{stem}.out.js` or `{stem}.out` beside the input. Before compile, `ensure_locked_for_entry` materialises locked packages; `--offline` is cache-only (miss is a hard error). JS emit uses the JS backend. `--library` is js-only: after the script body, named ESM exports are appended from IR metadata so Node can `import { name } from` the artifact. Without `--library`, emit stays a script (no `export {}` for an empty named-export table). Native emit uses LLVM with optional LTO and extra `.a` archives. `--strip` and `--lto` are native-only; `--link` is native-only. `--library` with `--target native` is a parse error. `--strip` runs host `strip` (or `$STRIP`) and removes a companion `.dSYM` on macOS. `--watch` rebuilds on mtime change. Help text omits `--offline`; `parse_build_args` accepts it.
