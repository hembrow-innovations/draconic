...
| E19.84.02 | todo | js | Deferred namespace identity/caching: same module's deferred ns shared across static `import defer` sites, re-export, and dynamic `import.defer`; distinct from eager ns (`identity.js`) | `tests/test262` (allowlist + linker) |
| E19.84.03 | todo | js | Deferred JSON-module namespace: `import defer * as ns from "...json" with { type: "json" }` compiles; `default` = parsed JSON (`json-module.js`) | `tests/test262` (allowlist + parser/linker) |
| E19.84.04 | todo | js | Re-exported deferred namespace preserves deferred semantics: `export { ns }` of defer ns stays lazy; `@@toStringTag` + evaluation triggers (`reexport-deferred-ns-evaluation.js`) | `tests/test262` (allowlist + linker) |
