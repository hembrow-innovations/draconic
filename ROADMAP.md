...
| E19.07 | todo | js | Checker: BigInt mixed-type operators (do not reject ECMA-262 `bigint`×`number`/`object`/`any` bitwise/arithmetic at compile when untyped; TypeError is runtime) | `crates/draconic-check`, `tests/test262` |
| E19.08 | todo | js | Assignment destructuring depth (Test262 `expressions/assignment/dstr`): remaining patterns beyond E18.19/E18.25 allowlist | `tests/test262` (allowlist + baseline-report) |
