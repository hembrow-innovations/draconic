...
| N08.16.26 | blocked | native | Real native observations: class public fields (E18.26) — `es/annex-b/class_fields` asserts program results (`native.stdout`), not B08 hello | fixture `es/annex-b/class_fields`, `crates/draconic-backend-llvm` |
...
| N08.16.29 | blocked | native | Real native observations: `export * from` (E18.29) — `es/annex-b/export_star` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/annex-b/export_star`, `crates/draconic-backend-llvm` |
...
| N08.16.33 | blocked | native | Real native observations: class expressions (E18.33) — `es/annex-b/class_expr` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/annex-b/class_expr`, `crates/draconic-backend-llvm` |
...
| N08.16.35 | blocked | native | Real native observations: async methods (E18.34) — `es/annex-b/async_methods` needs native.stdout off hello | fixture `es/annex-b/async_methods`, `crates/draconic-backend-llvm` — **blocked**: LLVM `es_promise` path only lowers async function/arrow + Promise ABI (N06.03–N06.11); fixture requires object `{ async m() }`, class instance/static `async` methods, `this` in async bodies, user `new Ctor`, and method-call this-binding. Integrating object/class + async/this into the Promise lowerer (or a new combined adapter) is a large job-queue feature, not a min native-observation enable. Unblock when native supports that surface. |
| N08.16.36 | blocked | native | Real native observations: private instance fields (E18.35) — `es/annex-b/private_fields` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/annex-b/private_fields`, `crates/draconic-backend-llvm` |
| N08.16.37 | blocked | native | Real native observations: static private fields (E18.36) — `es/annex-b/static_private_fields` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/annex-b/static_private_fields`, `crates/draconic-backend-llvm` |
| N08.16.38 | blocked | native | Real native observations: private instance methods (E18.37) — `es/annex-b/private_methods` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/annex-b/private_methods`, `crates/draconic-backend-llvm` |
| N08.16.39 | blocked | native | Real native observations: static private methods (E18.38) — `es/annex-b/static_private_methods` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/annex-b/static_private_methods`, `crates/draconic-backend-llvm` |
| N08.16.40 | blocked | native | Real native observations: private accessors (E18.39) — `es/annex-b/private_accessors` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/annex-b/private_accessors`, `crates/draconic-backend-llvm` |
...
| N08.16.42 | blocked | native | Real native observations: class static initialization blocks (E18.41) — `es/annex-b/static_blocks` asserts program results (`native.stdout`), not B08 hello | fixture `es/annex-b/static_blocks`, `crates/draconic-backend-llvm` |
| N08.16.43 | blocked | native | Real native observations: `for await…of` (E18.42) — blocked: native async iteration too large for one loop (needs `for await` in LLVM Promise path + `Symbol.asyncIterator` / CreateAsyncFromSyncIterator + object-method async iterables + break/continue; fixture `es/annex-b/for_await_of` remains js-only) | `tests/conformance` fixture `es/annex-b/for_await_of`, `crates/draconic-backend-llvm` |
...
| N08.16.46 | blocked | native | Real native observations: private residual (E19.36) — `es/annex-b/private_residual` asserts program results (`native.stdout`), not B08 hello | `tests/conformance` fixture `es/annex-b/private_residual`, `crates/draconic-backend-llvm` |
