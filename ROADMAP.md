...
| N08.07 | todo | native | Real native observations: strings (E07) (children below) | `tests/conformance` fixtures `es/strings` |
...
| N08.07.02 | todo | native | Real native observations: template literals (E07.02) — `es/strings/template_lit` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/strings/template_lit`, `crates/draconic-backend-llvm` |
| N08.07.03 | todo | native | Real native observations: unicode escapes (E07.03) — `es/strings/unicode_escapes` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/strings/unicode_escapes`, `crates/draconic-backend-llvm` |
| N08.07.04 | todo | native | Real native observations: tagged templates (E07.04) — `es/strings/tagged_template` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/strings/tagged_template`, `crates/draconic-backend-llvm` |
| N08.07.05 | todo | native | Real native observations: UTF-16 code-unit semantics (E07.05) — `es/strings/utf16_semantics` asserts program results (native.stdout), not B08 hello | `tests/conformance` fixture `es/strings/utf16_semantics`, `crates/draconic-backend-llvm` |
